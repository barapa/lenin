function paramsp = hessian_free_train( runDesc, paramsp, Win, bin,...
...
model_dir = '/var/data/lenin/hessian_free_models/';
...
for epoch = epoch:maxepoch
    ...
    tmp2 = ch;
    ch = single(ch);
    save( [model_dir runName '_nnet_running.mat'], 'paramsp', 'ch', 'epoch', 'lambda', 'totalpasses', 'llrecord', 'times', 'errrecord', 'lambdarecord' );
    if mod(epoch,10) == 0
        save( [model_dir runName '_nnet_epoch' num2str(epoch) '.mat'], 'paramsp', 'ch', 'epoch', 'lambda', 'totalpasses', 'llrecord', 'times', 'errrecord', 'lambdarecord' );
    end
    paramsp = tmp;
    ch = tmp2;
