...
layersizes = [1000, 800, 500, 200, 50]; % do not include output or input layer

% choices: 'logistic', 'softmax', 'linear'
layertypes = {'logistic', 'logistic', 'logistic',...
              'logistic', 'logistic', 'softmax' }; % do not include input layer
...
train_file_names = get_song_data_full_paths(train_song_names, window_size,...
    window_overlap, nfft);
validation_file_names = get_song_data_full_paths(validation_song_names, window_size,...
    window_overlap, nfft);
...
[validation_data, ~, validation_one_hot_labels] = load_songs(validation_file_names);
...
validation_data = construct_features_with_left_and_right_frames(validation_data,...
    left_frames_network, right_frames_network);
...
total_validation_frames = total_validation_frames - mod(total_validation_frames, numchunks_test);
