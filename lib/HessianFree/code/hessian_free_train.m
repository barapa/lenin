function [paramsp, layersizes, layertypes, model_name] = hessian_free_train(...
    runDesc,...
    paramsp,...
    Win,...
    bin,...
...
%
% initlambda: Default before was to set to 45. It should be set so that the 
%             value of rho is betweem 0.75 and 0.95 after the first epoch.
%
% save_params: a struct containing a bunch of stuff needed to save the best models
%              from within this function as they are found.

% set the runName to the filename we are going to save this in
runName = ['shf_', datestr(now, 'yyyymmddTHHMMSS')];
model_name = runName;
save_params.model_name = model_name;

model_dir = ['/var/data/lenin/hessian_free_models/' model_name '/'];
...
save_params.maxepoch = maxepoch;
save_params.rms = rms;
save_params.mattype = mattype;
save_params.decay = decay;
...
if mod( numcases, numchunks ) ~= 0
    error( [' Number of training cases: ', num2str(numcases)]);
...
fid = fopen( [model_dir runName '.txt'], 'a' );
...
for epoch = epoch:maxepoch
    ...
    tmp2 = ch;
    ch = single(ch);
    save( [model_dir runName '_nnet_running.mat'], 'paramsp', 'ch', 'epoch',...
        'lambda', 'totalpasses', 'llrecord', 'times', 'errrecord',...
        'lambdarecord', 'layersizes', 'layertypes',...
        'runDesc', 'maxepoch', 'numchunks',...
        'numchunks_test', 'mattype', 'rms', 'errtype',...
        'hybridmode', 'weightcost', 'decay', 'jacket');
    if mod(epoch,10) == 0
        save( [model_dir runName '_nnet_epoch' num2str(epoch) '.mat'],...
            'paramsp', 'ch', 'epoch',...
            'lambda', 'totalpasses', 'llrecord', 'times', 'errrecord',...
            'lambdarecord', 'layersizes', 'layertypes',...
            'runDesc', 'maxepoch', 'numchunks',...
    ...
    if err_test == min(err_record)
      model_save_pathname = save_hf_model(...
        save_params.model_name,...
        save_params.train_song_names,...
        save_params.validation_song_names,...
        save_params.test_song_names,...
        save_params.standardize_params,...
        paramsp,...
        ...
        save_params.runDesc,...
        save_params.errtype,...
        save_params.weightcost,...
        save_params.left_frames_network,...
        save_params.right_frames_network,...
        save_params.maxepoch,...
        save_params.rms,...
        save_params.mattype,...
        save_params.decay,...
        ll,...
